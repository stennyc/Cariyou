package com.qixiaoyi.cariyou.widget;
...
public class MyScrollview extends ScrollView {
    ...
    //当xml解析完毕加载成了View之后调用   自定义控件常实现的方法
    @Override
    protected void onFinishInflate() {
        int childCount = getChildCount();
        if (childCount > 0) {
            innerView = getChildAt(0);//ScrollView只能有一个孩子 根布局
        }
    }
    ...
    private boolean isNeedMove() {
        int offset = innerView.getMeasuredHeight() - getHeight();//布局高度减去屏幕显示高度
